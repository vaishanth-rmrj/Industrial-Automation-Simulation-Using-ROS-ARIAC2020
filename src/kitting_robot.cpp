...
void KittingRobot::init()
{
    ...
    // conveyor part pose sub
    conveyor_part_pose_sub = nh.subscribe("/group2/conveyor_part_pose", 10, &KittingRobot::conveyor_part_pose_sub_callback, this);
...
void KittingRobot::conveyor_part_pose_sub_callback(const geometry_msgs::Pose::ConstPtr& part_pose)
{   
    conveyor_part_pose = *part_pose;
}
...
bool KittingRobot::pickup_conveyor_part(double part_offset)
{
    activate_gripper();    

    geometry_msgs::Pose empty_pose;
    // check if the part is below the conveyor logical camera
    if(conveyor_part_pose == empty_pose)
    {   
        ROS_INFO("Part still not in range");
        ros::Duration(2).sleep();
        if(conveyor_part_pose == empty_pose)
        {
            ROS_INFO("Terminating conveyor part pickup");
            return false;
        }
    }
    ...
    geometry_msgs::Pose part_init_pose_in_world = conveyor_part_pose;
    part_init_pose_in_world.position.y = part_init_pose_in_world.position.y - 0.15;
    part_init_pose_in_world.position.z = part_init_pose_in_world.position.z + part_offset - 0.005;
    part_init_pose_in_world.orientation.x = arm_ee_link_pose.orientation.x;
    part_init_pose_in_world.orientation.y = arm_ee_link_pose.orientation.y;
    part_init_pose_in_world.orientation.z = arm_ee_link_pose.orientation.z;
    part_init_pose_in_world.orientation.w = arm_ee_link_pose.orientation.w;
    ...
    if (state.enabled) {
        ...
        // move the arm closer until the object is attached
        state = get_gripper_state();    
        double conveyor_pickup_start_time =ros::Time::now().toSec();
        while (!state.attached) {
            
            part_init_pose_in_world.position.x = conveyor_part_pose.position.x;
            part_init_pose_in_world.position.y = conveyor_part_pose.position.y - 0.13;
            part_init_pose_in_world.position.z = part_init_pose_in_world.position.z - 0.001;   

            if(conveyor_part_pose.position.y < 0.5)
            {
                go_to_location(arm_preset_at_conveyor);  
            ...
            if(conveyor_pickup_start_time + 10.0 == ros::Time::now().toSec())
            {
                ROS_INFO("Terminating conveyor part pickup");
                break;
            }   
        ...
        part_init_pose_in_world.position.y -= 0.04;
        part_init_pose_in_world.position.z += 0.20;
        kitting_robot_moveit_group.setPoseTarget(part_init_pose_in_world);
        kitting_robot_moveit_group.move();
...
bool KittingRobot::place_part(geometry_msgs::Pose target_pose)
{
    ...
    ROS_INFO_STREAM("Placing the part at "<< part_in_world_frame);     
...
{   
    ...
    if(req.initial_preset_location == "conveyor")
    {
        kitting_robot_initial_joint_pose = arm_preset_at_conveyor;
    ...
    if(req.initial_preset_location == "conveyor")
    {
        ROS_INFO("Moving robot to conveyor");
        go_to_location(kitting_robot_initial_joint_pose);       

        if(pickup_conveyor_part(req.part_offset)) 
        {
            // perform kitting
            go_to_location(kitting_robot_final_joint_pose);  

            ROS_INFO_STREAM("Placing the part at "<< req.part_target_pose);         
